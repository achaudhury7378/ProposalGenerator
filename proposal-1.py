...
# from webscrapper import get_links
import random
...
def tavily_search(query: str, max_results: int = 1) -> str:
    client = TavilyClient(api_key="***REMOVED***")
    response = client.search(query=query, max_results=1)
    lines = []
    for result in response.get("results", []):
        # lines.append(f"Title: {result['title']}\nURL: {result['url']}\nContent: {result['content']}\n")
        lines.append(f"Title:{result['title']} Long Content: {extract_text_from_url(result['url'])} Summery:{result['content']}")
    return "\n".join(lines)
...
model_client = OpenAIChatCompletionClient(
    ...
    api_key="***REMOVED***",         # your real OpenAI API key
)
##################PROMPTS   ###############################################
import configparser
...
################### TASK #################################################
task = f"""Query {random.randint(0,1000)}: can you write a proposal on topic:Invntory Management in Supply Chains  ? """
